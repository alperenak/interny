...
class MyTasks extends Component {
  ...
  getTasks = async () => {
    ...
    if (user === "intern") {
      ...
      internList = internList.map((val) => {
        const selected = this.state.selectedInterns.filter(
          (id) => id === val.id
        );
      ...
      if (this.state.selectedInterns.length > 0)
        res = await store.getEmployerTasksForInternID(
          this.props.selectedJobID,
          this.state.selectedInterns.join(",")
        );
      else res = await store.getTasks(this.props.selectedJobID);
    }
  ...
  onCreateClick = async () => {
    ...
    this.props.createModal({
      header: "Create Task",
      content: () => this.renderCreateTaskForm(createItems),
    });
  };
  ...
  onEditClick = async (item) => {
    ...
    this.props.createModal({
      header: "Edit Task",
      content: () => this.renderEditTaskForm(editItems),
    });
  };
          ...
          return (
            <div key={item.key + i} className={item.key}>
              <Input
                ...
                selecedInternsForMultiSelect={
                  this.state.selecedInternsForMultiSelect
                }
                setSelecedInternsForMultiSelect={this.setSelecedInternsForMultiSelect.bind(
                  this
                )}
        ...
        <Button
          ...
          onButtonClick={async () => {
            if (btn.key === "create") {
              ...
              else await this.onCreateFormSubmit(this.state.createFormData);
            }
  ...
  onEditFormSubmit = async (payload) => {
    ...
    if (
      typeof newPayload.label !== "string" &&
      Array.isArray(newPayload.label)
    ) {
      newPayload.label = payload.label.value;
    }
  ...
  toggleIntern(internInfo) {
    ...
    const internSelected = this.state.selectedInterns.filter(
      (id) => id === internInfo.id
    );
    if (Array.isArray(internSelected))
      alreadySelected = internSelected.length > 0;
    let internList = this.state.internList;

    const not_this_intern_from_selected = this.state.selectedInterns.filter(
      (id) => id != internInfo.id
    );
    let selectedInterns = [];

    if (alreadySelected) {
      selectedInterns = [...not_this_intern_from_selected];
    } else {
      selectedInterns = [...not_this_intern_from_selected, internInfo.id];
    }
  ...
  searchInInternList(query = "") {
    ...
    if (
      !(
        query.length >= searchOptions.length.min &&
        query.length <= searchOptions.length.max
      )
    )
      return;
    searchResults = [];
    ...
    searchResults = internList.filter((v) => {
      if (!(typeof v.name === "string" && typeof v.surname === "string"))
        return false;

      const searchfc = (searchIn) => {
        if (typeof searchIn.search === "undefined") return false;
        return (
          searchIn.search(query) != -1 ||
          searchIn.toLowerCase().search(query.toLowerCase()) != -1
        );
      };
